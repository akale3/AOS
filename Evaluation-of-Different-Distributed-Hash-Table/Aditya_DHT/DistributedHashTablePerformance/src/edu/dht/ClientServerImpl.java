	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				switch (operation) {
				case "1":
					boolean isSuccess = put(entry.substring(1, 11).trim(), entry.substring(11).trim());
					objectOutputStream.writeObject(isSuccess);
					objectOutputStream.flush();
					break;
				case "2":
					String value = get(entry.substring(1, 11).trim());
					objectOutputStream.writeObject(value);
					objectOutputStream.flush();
					break;
				case "3":
					boolean isDeleted = delete(entry.substring(1, 11).trim());
